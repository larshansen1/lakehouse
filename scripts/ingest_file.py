#!/usr/bin/env python3
"""Stage local datasets into MinIO landing zone and refresh bronze tables."""
...
def ingest_dataset(*, args: argparse.Namespace, env_config: EnvConfig, duckdb_binary: str) -> None:
    ...
    statements.append(ducklake_attach_statement(env_config))
    ...
    append_drop_statement(statements, schema="bronze", name=latest_view, relation_type=latest_relation_type)
    append_drop_statement(statements, schema="bronze", name=table_name, relation_type=table_relation_type)
    statements.append(
    ...
    run_duckdb_with_retry(statements, duckdb_binary=duckdb_binary)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="business_keys",
        action="append",
        help=f"Column(s) that define the business key for latest view (default: {DEFAULT_BUSINESS_KEY}). "
